...
bool BuyOrangeGFLayer::init()
{
	...
	else
	{
		...
		descstr = StringUtils::format("%s%s%s%d%s", CommonFuncs::gbk2utf("当前已拥有“").c_str(), GlobalData::map_allResource[vec_tmpqu5gf[r]].cname.c_str(), CommonFuncs::gbk2utf("”，自动转换成").c_str(), 60 , CommonFuncs::gbk2utf("”元宝").c_str());
	}
